...
test('Medication Edit Screen render correctly', async ()=>{
    jest.spyOn(firebase,'firestore').mockImplementationOnce(()=> firestoreMock);
    const wrapper = renderer.create(
        <FirebaseAuthProvider>
            <EditMedicationScreen navigation={navigationProp} route={routeProp} />
        </FirebaseAuthProvider>
    );
    expect(wrapper.toJSON()).toMatchSnapshot();
});

test('Medication Add Screen render correctly', async ()=>{
    const wrapper = renderer.create(
        <FirebaseAuthProvider>
            <AddMedicationScreen navigation={navigationProp} route={routeProp} />
        </FirebaseAuthProvider>
    );
    expect(wrapper.toJSON()).toMatchSnapshot();
});